  ...
  return {
    ...
    <sc-sample-box title="첨부 버튼 눌렀을때 validation 후 처리가 필요한 경우">
      <template #description>
        :before-add-validation 첨부버튼 눌렀을때 검증이 필요한 경우 해당 속성을 이용하여 처리 가능
      </template>
      해당 Sample은 예제를 위해 작성되었으며 실제 업로드 되지는 않습니다.
      <br /><br />
      <sc-file-uploader
  ...
  return {
    ...
    <sc-sample-box title="버튼 영역 추가 ">
      <template #description>
        ...
        slot:rightButton - 첨부 버튼 기준 오른쪽으로 버튼 추가 영역
      </template>
      해당 Sample은 예제를 위해 작성되었으며 실제 업로드 되지는 않습니다.
      <br /><br />
...
<template #rightButton>
  ...
  <sc-button small>Custom Right 2</sc-button>
    ...
    <sc-sample-box title="Download Usage">
      해당 Sample은 예제를 위해 작성되었으며 실제 다운로드 되지는 않습니다.
      <br /><br />
      <sc-file-uploader
        :init-list="fakeFileList"
        :only-download="true"
...
export default {
  name: 'SampleFileForm',
  components: { ScButton, ScCodeHighlight, ScFileUploader, ScSampleBox },
  methods: {
    onFileHandle(flag, event) {
      console.info(flag, event);
    },
